...
export default function TabBar({ activeTab, onTabChange }: TabBarProps) {
  return (
    <nav className="bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-700 overflow-x-auto">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-stretch h-10 min-w-max sm:min-w-0">
        {TABS.map((tab, index) => (
          <div key={tab.id} className="flex items-stretch">
            ...
            <button
              ...
              className={`px-3 sm:px-5 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
                activeTab === tab.id
